...
$file = [];
$file_s = CFile::ResizeImageGet($arResult['DETAIL_PICTURE'], array('width' => 125, 'height' => 113), BX_RESIZE_IMAGE_PROPORTIONAL, true);
$file_b = CFile::ResizeImageGet($arResult['DETAIL_PICTURE'], array('width' => 300, 'height' => 273), BX_RESIZE_IMAGE_PROPORTIONAL, true);
$file[] = [
...
if (is_array($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'])) {
    foreach ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $item) {
        $file_orig = CFile::GetFileArray($item);
        $file_sm = CFile::ResizeImageGet($item, array('width' => 125, 'height' => 113), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        $file_big = CFile::ResizeImageGet($item, array('width' => 300, 'height' => 273), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        $file[] = [
            'ORIGINAL' => $file_orig,
            'SMALL' => $file_sm,
            'BIG' => $file_big
...
/*if(!empty($_SESSION['viewed'])){
    $arFilter = Array("IBLOCK_ID"=>$arResult['IBLOCK_ID'], 'ID' => $_SESSION['viewed'], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
...
if (is_array($arResult['PROPERTIES']['BUYWITH']['VALUE'])) {
    $arFil = Array("IBLOCK_ID" => $arResult['IBLOCK_ID'], 'ID' => $arResult['PROPERTIES']['BUYWITH']['VALUE'], "ACTIVE_DATE" => "Y", "ACTIVE" => "Y");
    $res2 = CIBlockElement::GetList(Array(), $arFil, false, Array("nPageSize" => 4), $arSelect);
...
$limitTabs = empty($arParams['LIMIT_ELEMENT_TAB']) ? $arDefaultParams['LIMIT_ELEMENT_TAB'] : $arParams['LIMIT_ELEMENT_TAB'];

if (is_array($arResult['PROPERTIES']['CERTIFICATE']['VALUE']) && ($items = $arResult['PROPERTIES']['CERTIFICATE'])) {

    $arFil = Array(
        "IBLOCK_ID" => $items["LINK_IBLOCK_ID"],
        'ID' => $items['VALUE'],
        "ACTIVE_DATE" => "Y",
        "ACTIVE" => "Y"
    );
    ...
    $res = CIBlockElement::GetList(
        ...
        $arFil,
        false,
        Array("nPageSize" => $limitTabs),
        array("ID", "IBLOCK_ID", "PREVIEW_PICTURE", "NAME")
    );
    ...
    while ($ob = $res->GetNextElement()) {
        ...
        $arItems[] = [
            ...
            "BLOCK_FILE" => $arProps["BLOCK_FILE"]["VALUE"],
            "NAME" => $arFields["NAME"],
            "BLOCK_BRAND" => $arProps["BLOCK_BRAND"]["VALUE"],
            "ID" => $arFields["ID"],
        ];
    }
    $arResult['TABS']["CERTIFICATE"] = $arItems;
}

if (is_array($arResult['PROPERTIES']['INSTRUCTIONS']['VALUE']) && ($items = $arResult['PROPERTIES']['INSTRUCTIONS'])) {
    ...
    $arResult['TABS']["INSTRUCTIONS"] = $arItems;
}

if (is_array($arResult['PROPERTIES']['CATALOGS']['VALUE']) && ($items = $arResult['PROPERTIES']['CATALOGS'])) {
    ...
    $arResult['TABS']["CATALOGS"] = $arItems;
}
