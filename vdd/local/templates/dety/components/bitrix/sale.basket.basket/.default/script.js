...
BX.ready(function() {
    ...
    if(checkAll) {
        checkAll.addEventListener('click',() => {
            checkAll.classList.contains('active') ? checkAll.classList.remove('active') : checkAll.classList.add('active')
            let checkProduct = document.querySelectorAll('.cart-item__check input');
    ...
    function ClearCart(element) {
        ...
        if (href)
            $.get(href, function(data){
                $("#shopCart").html($(data).find("#shopCart".html())); //меняем на своё
            });
