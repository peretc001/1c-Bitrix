...
{
//    if ($arItem['PARAM2'] == 5) $arElement['OFFERS'][$i] = true;
    ...
    debug($arItem['ITEM_ID'],1);

    $arResult["SEARCH"][$i]["ICON"] = true;
}
//TODO: Сдеалть поиск по артикулу и вывод фото ТП
if (!$arResult["SEARCH"] && is_numeric($arResult['query'])) {
    ...
    $rsElements = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 4, 'ACTIVE' => 'Y', 'PROPERTY_CML2_ARTICLE' => '%'.$arResult['query'].'%'), false, false, $arSelect);
    foreach($arResult["SEARCH"] as $i=>$arItem):
        while($arElement2 = $rsElements->GetNext()):
            $arElement = &$arResult["ELEMENTS"][$arItem["ITEM_ID"]];

            $arElement["FIND_ARTNUMBER"][$i] = $arElement2;
        endwhile;
    endforeach;
}
